...
describe('mockComponent function', function() {
	...
	it('has reference to the mocked functions', function() {
		var componentPath = require.resolve('./mocks/ComponentOne.react');
		var Component = mockComponent(componentPath);
		expect(Component.mockedMethods.foo).not.toBeUndefined();
	});
	...
	it('can auto-mock functions with additional methods', function() {
		...
		expect(component.newMethod).toEqual(newMethod);
	});

	it('can auto-mock functions overwriting methods', function() {
		var componentPath = require.resolve('./mocks/ComponentOne.react');
		var newMethod = function() {};
		var Component = mockComponent(componentPath, 'MyTag', {
			foo: newMethod
		...
		expect(component.foo).toEqual(newMethod);
	});
